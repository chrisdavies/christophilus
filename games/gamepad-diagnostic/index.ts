  ...
  constructor(index: number, title: string) {
    ...
    this.append(
      h(
        ...
        h('xc-controller-header', title),
        h(
          'xc-controller',
          h('xc-dpad', h('xc-dpad-up'), h('xc-dpad-right'), h('xc-dpad-down'), h('xc-dpad-left')),
          h('xc-select'),
          h('xc-start'),
          h(
            'xc-buttons',
          ...
          h('xc-trigger-bottom-left'),
          h('xc-trigger-bottom-right'),

          h('xc-trigger-top-left'),
          h('xc-trigger-top-right'),
        ),
      ),
    );
  }
  connectedCallback() {
    const toggleButton = (gamepad: Gamepad, selector: string, ...index: number[]) => {
      this.querySelector(selector)?.classList.toggle(
        'xc-active',
        index.some((i) => gamepad.buttons[i]?.pressed),
      );
    };

    const setAxis = (selector: string, x: number, y: number) => {
...
class Page extends HTMLElement {
  instructions = h(
    'xc-instructions',
    'Connect one or more controllers and press any button to get started...',
  );
  ...
  onGamepadDisconnected = (e: GamepadEvent) => {
    const gamepads = this.querySelectorAll<GamepadDiagnostic>('xc-gamepad-diagnostic');
